...
public class PatientDaoImpl extends AbstractDao<PatientEntity, Long> implements PatientDao {
    ...
    public List<PatientEntity> findPatientsHavingTreatmentType(TreatmentType treatmentType) {
        return entityManager.createQuery(
                "SELECT DISTINCT p FROM PatientEntity p " +
                        "JOIN p.visits v " +
                        "JOIN v.medicalTreatments m " +
                        "WHERE m.type=:treatmentType", PatientEntity.class
        ).setParameter("treatmentType",treatmentType).getResultList();
    }
    ...
    public List<PatientEntity> findPatientsWithoutLocation() {
        return entityManager.createQuery("SELECT p FROM PatientEntity p " +
                        "WHERE SIZE(p.addresses)=0", PatientEntity.class)
                .getResultList();
    }
}
